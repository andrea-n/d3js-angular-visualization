...
export class GameAnalysisComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    setInterval(function () { this.loadData() }.bind(this), 5000);
  }
  ...
  loadData() {
    ...
    this.papa.parse("assets/user_events_log.csv", {
      download: true,
      header: true,
  ...
  drawPlan(config: any) {
    ...
    this.planDomain = d3.max(layers[layers.length - 1], function(d) { return d[1]; });
